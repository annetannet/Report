...
class TestGraphs(unittest.TestCase):
    def test_bfs_simple_graph(self):
        self.assertEqual(bfs.BFS(
            [[1, 2], [0, 2], [0, 1, 3], [2]], 0), [0, 1, 2, 3])
        self.assertEqual(bfs.BFS(
            [[1, 2, 3, 4, 5], [0], [0], [0], [0], [4]], 0), [0, 1, 2, 3, 4, 5])
    ...
    def test_bfs_node_out_of_graph(self):
        self.assertEqual(bfs.BFS(
            [[1, 20000], [0, 2], [0, 1, 3], [2]], 0), [])
        # The node 20000 out of graph
        self.assertEqual(bfs.BFS([[1, 2, 3, 4, 5], []], 0), [])
        # The node 2 out of graph
        self.assertEqual(bfs.BFS([[1], []], 10), [])
        # The node 10 out of graph
    ...
    def test_dfs_node_out_of_graph(self):
        ...
        # The node 4 out of graph

    def test_dfs_no_repetition_visits(self):
        self.assertEqual(dfs.DFS(
            [[1, 2], [3, 4], [1], [1], [0]], 0), [0, 1, 3, 4, 2])
        self.assertEqual(dfs.DFS(
            [[1, 2], [3, 4], [0], [1], [1]], 0), [0, 1, 3, 4, 2])
    ...
    def test_top_sort_node_out_of_graph(self):
        self.assertEqual(top_sort.TopologicalSort([[1, 2], [3], []]), [])
        # The node 3 out of graph
        self.assertEqual(top_sort.TopologicalSort([[1, 2]]), [])
        # The node 1 out of graph
        self.assertEqual(top_sort.TopologicalSort([[1, 2, 10], [], []]), [])
        # The node 10 out of graph

    def test_common_topological_sort(self):
        self.assertEqual(top_sort.TopologicalSort(
            [[1, 2], [3, 4], [3, 4], [5], [5], []]), [0, 2, 1, 4, 3, 5])
    ...
    def test_top_sort_DFS(self):
        ...
        self.assertEqual(top_sort_DFS.DFS([[1, 2], [2], [3], []]),
                         [0, 1, 2, 3])
